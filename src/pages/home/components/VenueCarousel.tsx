...
import CustomCarouselItem from "./VenueCard";
...
interface VenueCarouselProps {
  cardList: VenueCardProps[];
}

export default function VenueCarousel({ cardList }: VenueCarouselProps) {
    ...
    <div className="relative mb-16 lg:hidden">
      ...
      >
        <CarouselContent className="">
          {cardList.map((banner) => (
            <CarouselItem key={banner.idx} className="flex items-center justify-center">
      ...
      <div className="absolute right-0 -bottom-8 left-0 z-10 flex justify-center gap-0">
        {cardList.map((banner, index) => (
          <div key={banner.idx} className="relative w-[40px]">
            <button
              disabled={false}
